...
public class Calculateur {
	...
	public static void main(String[] args) throws CalculatriceException {
		...
		System.out.println("abs(1) = " + calculatrice.calculer("abs", 1.0));
		...
		Set<Operation> op = calculatrice.getOperation();
		System.out.println("Calculatrice a " +
							op.stream().filter(a -> a.getNbOperandes()==Operation.OperandeCardinalite.TWO).count() +
							" operations d'arite 2");
		
		op
			.stream()
			.filter(a -> a.getNbOperandes()==Operation.OperandeCardinalite.TWO)
			.forEach(a -> System.out.println("2.3 " + a.getNom() + " 5.4 = " +a.calculer(2.3,5.4)) );
	}
