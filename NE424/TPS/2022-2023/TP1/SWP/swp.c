...
static void send_nak_frame(uint_t seq) /** nouveau **/
{
  frame s;
...
void swp(int id)
{
  ...
  uint_t retrans_until=-1; /** nouveau **/
...
/*################################################################*/ 
    case network_layer_ready:   /* accept, save, and transmit a new frame */
      ...
      if(retrans_until !=-1 && !seq_between(sw2, sw1, retrans_until))/** nouveau **/
		  retrans_until = -1; /** nouveau **/
	  
	  if(retrans_until==-1) /** nouveau **/
		/* fetch new packet */
...
/*################################################################*/
	...
	inc_seq(sw1);    /* advance sender's window */
	  ...
	  if (r.kind == nak && seq_between(r.ack, sw1, sw2)) { 
		  nbuffered=0;
		  if(retrans_until == -1)retrans_until = sw2;
		  sw2 = sw1;		  
	  }
      break;
