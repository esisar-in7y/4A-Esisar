...
part_sizes, max_size = generate_bpin_stance(12, 37)
...
model.x = Var(model.n, model.m, within=Binary)  # Si l'objet i est mis dans la boîte j
...
def constraint_rule2(model, j):
    return sum(model.s[i] * model.x[i, j] for i in model.n) <= model.B * model.y[j]
